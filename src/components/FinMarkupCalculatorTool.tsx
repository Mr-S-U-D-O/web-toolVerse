...
export default function FinMarkupCalculatorTool({ onBack }: { onBack: () => void }) {
  ...
  const revenue = c + markupAmount;
  const marginPercent = revenue > 0 ? (markupAmount / revenue) * 100 : 0;
        ...
        <div className="max-w-4xl w-full mx-auto p-6 md:p-8">
          <div className="flex flex-col items-center justify-center mb-12 text-center">
            ...
            <p className="text-on-surface-variant max-w-2xl">
              Determine selling price and margin from cost and markup percentage.
            </p>
          </div>
          ...
          <div className="bg-surface-container rounded-3xl p-6 md:p-8 ring-1 ring-outline-variant">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-on-surface">Markup (%)</label>
                <input
                  type="number"
                  value={markupPercent}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                <div className="text-sm text-on-surface-variant mb-1">Selling Price</div>
                <div className="text-2xl font-bold text-on-surface font-mono">${revenue.toFixed(2)}</div>
              </div>
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                <div className="text-sm text-on-surface-variant mb-1">Markup Amount</div>
                <div className="text-2xl font-bold text-on-surface font-mono">${markupAmount.toFixed(2)}</div>
              </div>
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                <div className="text-sm text-on-surface-variant mb-1">Gross Margin</div>
                <div className="text-2xl font-bold text-on-surface font-mono">{marginPercent.toFixed(2)}%</div>
              </div>
            </div>
          </div>
        </div>
