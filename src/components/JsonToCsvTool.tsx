...
function escapeCsvCell(val: any, delimiter = ','): string {
...
function convertJsonToCsv(jsonStr: string, flatten = true, delimiter = ','): string {
...
export default function JsonToCsvTool({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    try {
      setOutputText(convertJsonToCsv(inputText, flatten, delimiter));
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to parse JSON.');
      setOutputText('');
    }
  }, [inputText, flatten, delimiter]);
    ...
    <div className="min-h-screen bg-background text-on-surface w-full pb-20 animate-in fade-in duration-300">
      ...
      <header className="sticky top-0 z-30 w-full border-b border-outline-variant bg-background/95 backdrop-blur-sm">
        <div className="max-w-[1280px] mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="font-sans text-sm font-medium text-on-surface">JSON to CSV</div>
      ...
      <main className="max-w-[1280px] mx-auto px-6 py-10 w-full">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="font-heading text-3xl font-bold tracking-tight text-[#008cff]">JSON to CSV</h1>
            <p className="text-on-surface-variant mt-2 text-sm font-sans max-w-2xl">
              Convert structured JSON arrays into standard tabular CSV files. Includes features for auto-flattening nested JSON properties and custom delimiters.
            </p>
          </div>
        </div>
          ...
          <div className="bg-surface-container-low p-5 rounded-2xl border border-outline-variant flex flex-col justify-between">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 bg-surface-container-high/40 p-4 border border-outline-variant/60 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="chk-flatten" className="font-sans text-xs text-on-surface-variant cursor-pointer select-none">
          ...
          <div className="bg-surface-container-low p-5 rounded-2xl border border-outline-variant flex flex-col justify-between">
            ...
            {errorMsg ? (
              <div className="w-full h-96 bg-red-500/5 border border-red-500/20 rounded-xl p-6 flex flex-col items-center justify-center text-center gap-3">
                ...
                <p className="font-sans text-sm font-semibold text-red-400">CSV Conversion Failed</p>
              ...
              <textarea
                readOnly
                value={outputText}
                placeholder="Converted CSV text will load here..."
