...
export default function TsvToJsonTool({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    try {
      const parsed = parseTsv(inputText);
      if (parsed.length === 0) {
        setErrorMsg('Could not detect headers or valid rows in the TSV input.');
        setOutputText('');
        return;
      }
      setOutputText(JSON.stringify(parsed, null, 2));
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to parse TSV.');
      setOutputText('');
    }
  }, [inputText]);
    ...
    <div className="min-h-screen bg-background text-on-surface w-full pb-20 animate-in fade-in duration-300">
      ...
      <header className="sticky top-0 z-30 w-full border-b border-outline-variant bg-background/95 backdrop-blur-sm">
        <div className="max-w-[1280px] mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="font-sans text-sm font-medium text-on-surface">TSV to JSON</div>
      ...
      <main className="max-w-[1280px] mx-auto px-6 py-10 w-full">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="font-heading text-3xl font-bold tracking-tight text-[#008cff]">TSV to JSON</h1>
            <p className="text-on-surface-variant mt-2 text-sm font-sans max-w-2xl">
              Convert tab-separated values (TSV) sheets or pasted logs into formatted JSON objects arrays. Handles double-quotes parsing and data type casting offline.
            </p>
          </div>
          ...
          <div className="bg-surface-container-low p-5 rounded-2xl border border-outline-variant flex flex-col justify-between">
            ...
            {errorMsg ? (
              <div className="w-full h-96 bg-red-500/5 border border-red-500/20 rounded-xl p-6 flex flex-col items-center justify-center text-center gap-3">
                ...
                <p className="font-sans text-sm font-semibold text-red-400">TSV Parsing Failed</p>
