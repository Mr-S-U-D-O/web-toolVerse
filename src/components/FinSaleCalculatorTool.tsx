...
export default function FinSaleCalculatorTool({ onBack }: { onBack: () => void }) {
        ...
        <div className="max-w-4xl w-full mx-auto p-6 md:p-8">
          <div className="flex flex-col items-center justify-center mb-12 text-center">
            ...
            <p className="text-on-surface-variant max-w-2xl">
              Calculate the final price of an item including discount and tax.
            </p>
          </div>
          ...
          <div className="bg-surface-container rounded-3xl p-6 md:p-8 ring-1 ring-outline-variant">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-on-surface">Original Price ($)</label>
                <input
                  type="number"
                  value={originalPrice}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-on-surface">Tax Rate (%)</label>
                <input
                  type="number"
                  value={taxPercent}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                <div className="text-sm text-on-surface-variant mb-1">Tax Amount</div>
                <div className="text-2xl font-bold text-on-surface font-mono">+${taxAmount.toFixed(2)}</div>
              </div>
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                <div className="text-sm text-on-surface-variant mb-1">You Saved</div>
                <div className="text-2xl font-bold text-on-surface font-mono">-${saved.toFixed(2)}</div>
              </div>
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline text-emerald-500 border-emerald-500/20 bg-emerald-500/10">
                <div className="text-sm text-emerald-500/80 mb-1">Final Price</div>
                <div className="text-2xl font-bold font-mono">${finalPrice.toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
