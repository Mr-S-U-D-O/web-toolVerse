...
function robustHTMLMinify(html: string): string {
    if (!html.trim()) return '';
    try {
        let temp = html;
        const protectedChunks: string[] = [];
        
        // Protect script, style, pre, code
        temp = temp.replace(/<(script|style|pre|code)[\s\S]*?<\/\1>/gi, (match) => {
            protectedChunks.push(match);
            return `___PROT${protectedChunks.length - 1}___`;
        });
        ...
        // Remove space between element tags
        temp = temp.replace(/>\s+</g, '><'); 
        temp = temp.trim();
        
        // Restore protected chunks
        for (let i = protectedChunks.length - 1; i >= 0; i--) {
            temp = temp.replace(`___PROT${i}___`, protectedChunks[i]);
        }
        
        return temp;
    } catch (err: any) {
        throw new Error(err.message || 'Failed to parse and minify HTML.');
    }
}
...
export default function HtmlMinifierTool() {
  ...
  const handleMinify = () => {
    ...
    try {
      const minified = robustHTMLMinify(input);
      setOutput(minified);
      setError(null);
    } catch (e: any) {
      setOutput('');
      setError(e.message || "An unexpected error occurred during HTML minification.");
    }
  };
        ...
        <div className="w-full max-w-5xl flex flex-col gap-8 animate-in fade-in duration-300">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-heading text-3xl font-semibold tracking-tight">HTML Minifier</h1>
                <p className="text-on-surface-variant mt-1 text-sm">Compress HTML payload while protecting scripts, styles, and format-sensitive tags.</p>
              </div>
            </div>
            ...
            >
              <Minimize2 className="w-4 h-4" /> Minify HTML
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 min-h-[450px]">
             ...
             <div className="bg-surface-container-low border border-outline-variant rounded-xl overflow-hidden shadow-sm flex flex-col h-[400px] md:h-[500px]">
                <div className="h-14 border-b border-outline-variant bg-surface-container/50 flex items-center px-5">
                  <span className="font-mono text-xs text-outline font-semibold uppercase tracking-widest">Raw HTML Input</span>
                </div>
                <textarea
                  value={input}
                  onChange={(e) => setInput(e.target.value)}
                  placeholder="Paste your uncompressed HTML here..."
