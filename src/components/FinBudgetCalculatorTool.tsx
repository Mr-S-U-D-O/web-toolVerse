...
export default function FinBudgetCalculatorTool({ onBack }: { onBack: () => void }) {
        ...
        <div className="max-w-4xl w-full mx-auto p-6 md:p-8">
          <div className="flex flex-col items-center justify-center mb-12 text-center">
            ...
            <p className="text-on-surface-variant max-w-2xl">
              Quickly calculate your budget deficit or surplus.
            </p>
          </div>
          ...
          <div className="bg-surface-container rounded-3xl p-6 md:p-8 ring-1 ring-outline-variant">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-on-surface">Total Income</label>
                <input
                  type="number"
                  value={income}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-on-surface">Total Expenses</label>
                <input
                  type="number"
                  value={expenses}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                ...
                <div className={`text-2xl font-bold font-mono ${remaining >= 0 ? 'text-emerald-500' : 'text-error'}`}>
                  {status}
                </div>
              </div>
              <div className="bg-surface-container-high p-4 rounded-xl border border-outline">
                <div className="text-sm text-on-surface-variant mb-1">Remaining Balance</div>
                <div className="text-2xl font-bold text-on-surface font-mono">${remaining.toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
