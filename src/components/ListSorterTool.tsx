...
import { ArrowLeft, ListOrdered, ArrowDownAZ, ArrowUpZA, Trash2, Copy, Check } from 'lucide-react';

export default function ListSorterTool({ onBack }: { onBack: () => void }) {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [copied, setCopied] = useState(false);

  const handleSort = (descending: boolean) => {
        ...
        <div className="w-full max-w-4xl flex flex-col gap-6 animate-in fade-in duration-300">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="font-heading text-2xl font-semibold tracking-tight">List Sorter</h1>
            </div>
            <div className="flex items-center gap-2">
              <button 
                onClick={() => handleSort(false)}
                className="flex items-center gap-2 bg-surface-container-highest hover:bg-surface-container border border-outline-variant px-3 py-1.5 rounded-md font-mono text-xs uppercase tracking-wider transition-colors shadow-sm"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6 min-h-[400px]">
             <div className="bg-surface-container-low border border-outline-variant rounded-xl overflow-hidden shadow-sm flex flex-col h-[400px] md:h-full">
                <div className="h-12 border-b border-outline-variant bg-surface-container flex items-center px-4">
                  <span className="font-mono text-xs text-outline uppercase tracking-widest">Unsorted List</span>
                </div>
                <textarea
                  value={input}
             ...
             <div className="border border-outline-variant bg-surface-container-low rounded-xl overflow-hidden shadow-sm flex flex-col h-[400px] md:h-full">
                <div className="h-12 border-b border-outline-variant bg-surface-container flex items-center px-4">
                  <span className="font-mono text-xs text-outline uppercase tracking-widest">Sorted List</span>
                </div>
                <textarea
                  value={output}
                  readOnly
