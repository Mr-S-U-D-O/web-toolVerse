...
export default function TextParagraphCounterTool({ onBack }: { onBack: () => void }) {
  const [text, setText] = useState<string>('');

  const countParagraphs = (str: string) => {
    if (!str.trim()) return 0;
    // splitting by newline characters and filtering out empty paragraphs
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-8">
        <div className="flex flex-col items-center justify-center mb-12 text-center">
          ...
          <p className="text-on-surface-variant max-w-2xl">
            Count the exact number of paragraphs in your text content.
          </p>
        </div>
        ...
        <div className="bg-surface-container rounded-3xl p-6 md:p-8 ring-1 ring-outline-variant">
          <div className="flex justify-center mb-6">
            <div className="w-full max-w-sm bg-surface-container-high p-4 rounded-xl border border-outline border-l-4 border-l-primary/50 text-center">
              <div className="text-sm text-on-surface-variant mb-1">Paragraph Count</div>
              <div className="text-4xl font-bold text-on-surface">{paragraphCount}</div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              placeholder="Paste or type your text here. A new paragraph starts after every line break."
            />
          </div>
        </div>
      </div>
